...
@Slf4j
public final class EventStreamReaderFixed<Type> implements EventStreamReader<Type> {
    ...
    EventStreamReaderFixed(SegmentInputStreamFactory inputStreamFactory,
                           ...
                           ImmutableMap<Stream, WatermarkReaderImpl> waterMarkReaders, Controller controller, long segmentID) {
     ...
     * positions. Any attempt to add/remove elements from the list results in {@link UnsupportedOperationException}.
     *
...
//                    System.out.println(this + " Failed to release segment " + segment);
    ...
    private boolean acquireSegmentsIfNeeded(PositionInternal position) throws ReaderNotInReaderGroupException {
        Map<SegmentWithRange, Long> newSegments = groupState.acquireSegmentById(getLag(), position, segmentID);
