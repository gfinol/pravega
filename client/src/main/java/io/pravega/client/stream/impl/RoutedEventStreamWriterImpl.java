...
public final class RoutedEventStreamWriterImpl<RoutingKey, Type> implements EventStreamWriter<Type> {
    ...
    RoutedEventStreamWriterImpl(Stream stream, String writerId, Controller controller, SegmentOutputStreamFactory outputStreamFactory,
                                Serializer<Type> serializer, EventWriterConfig config, ExecutorService retransmitPool,
                                ScheduledExecutorService internalExecutor, ConnectionPool connectionPool, Function<RoutingKey, Double> hashFunction) {
    ...
    public CompletableFuture<Void> writeRoutedEvent(Type event) {
        return writeRoutedEvent(null, event);
    }

    public CompletableFuture<Void> writeRoutedEvent(RoutingKey routingKey, Type event) {
    ...
    private CompletableFuture<Void> writeRoutedEventInternal(RoutingKey routingKey, Type event) {
        ...
        synchronized (writeFlushLock) {
            if (config.isEnableLargeEvents() && data.remaining() > Serializer.MAX_EVENT_SIZE) {
//                writeLargeEvent(routingKey.toString(), Collections.singletonList(data), ackFuture);
                throw new UnsupportedOperationException("Large events are not supported for RoutedEventStreamWriter");
            } else {
                synchronized (writeSealLock) {
                    SegmentOutputStream segmentWriter = getRoutedSegmentWriter(routingKey);
                    segmentWriter.write(PendingEvent.withHeader(routingKey.toString(), data, ackFuture));
                }
            }
        }
        return ackFuture;
